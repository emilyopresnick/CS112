...
public class Puzzle {
    ...
    //helper method that returns true if it is safe to place a value and false other wise
    private boolean isSafe(int row, int col, int val){
        if(this.rowHasValue[row][val]||this.colHasValue[col][val]||this.subgridHasValue[row/3][col/3][val]){
            return false;
        }
        return true;
    }  
