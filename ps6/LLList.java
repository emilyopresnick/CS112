...
public class LLList implements List {
    ...
    public boolean addItem(Object item, int i) {
        ...
        length++;
        if(last.next!=null) last=newNode;
        return true;
    }
    ...
    public Object removeItem(int i) {
        ...
        if(i==length-1)  last=prevNode;
        length--;
        return removed;

    }

    //rotates LLList objects nodes k times
    private void rotate(int k) {
        if(k<0 || k>length){
            throw new IllegalArgumentException("can not rotate by this number");
        }
        Node oldHead = head.next;
        head.next=getNode(length-k);
        last.next=oldHead;
